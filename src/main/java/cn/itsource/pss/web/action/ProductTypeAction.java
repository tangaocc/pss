...
import cn.itsource.pss.query.ProductTypeQuery;
...
public class ProductTypeAction extends CRUDAction<ProductType>{
	...
	private PageList<ProductType>  pageList;
	...
	public String save(){
		...
		productTypeService.save(productType);// 当productType保存之后，productType就变为是一个持久状态，持久状态有id
		return RELOAD;
	}
	 ...
	 * 
	 * 		判断用户名是否存在：
	 * 		判断新增还是修改，我们可以通过productType的主键id进行一个判断
	 * 				1. 新增
						  验证用户名是否存在
							1.拿到当前用户名去数据库中进行查询
							2.如果查询的总数>0,那证明就已经存在了
	 ...
	 * 				2. 修改
	 * 					 通过id去把对应的productType对象查询出来
	 * 						1.username与productType对应的 username进行一个标胶
	...
	public String checkName() throws IOException{
		HttpServletResponse response = ServletActionContext.getResponse();
		response.setContentType("text/json;charset=utf-8");
		response.getWriter().println("{\"valid\":"+productTypeService.checkName(id,name)+",\"message\":\"用户名重复了\"}");
		return NONE;
	}
	...
	public void prepareInput() throws Exception {
		if(id != null){
			productType = productTypeService.queryOne(id);
		}
	}
