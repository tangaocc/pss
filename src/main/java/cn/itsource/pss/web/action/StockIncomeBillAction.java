...
import cn.itsource.pss.query.StockIncomeBillQuery;
...
public class StockIncomeBillAction extends CRUDAction<StockIncomeBill>{
	...
	private PageList<StockIncomeBill>  pageList;
	...
	protected void list() throws Exception {
		//查询员工列表
		pageList = stockIncomeBillService.findPageByQuery(baseQuery);
	}
	...
	public String save(){
		...
		stockIncomeBillService.save(stockIncomeBill);// 当stockIncomeBill保存之后，stockIncomeBill就变为是一个持久状态，持久状态有id
		return RELOAD;
	}
	 ...
	 * 
	 * 		判断用户名是否存在：
	 * 		判断新增还是修改，我们可以通过stockIncomeBill的主键id进行一个判断
	 * 				1. 新增
						  验证用户名是否存在
							1.拿到当前用户名去数据库中进行查询
							2.如果查询的总数>0,那证明就已经存在了
	 ...
	 * 				2. 修改
	 * 					 通过id去把对应的stockIncomeBill对象查询出来
	 * 						1.username与stockIncomeBill对应的 username进行一个标胶
	...
	public void prepareInput() throws Exception {
		if(id != null){
			stockIncomeBill = stockIncomeBillService.queryOne(id);
		}
	}
	...
	public PageList<StockIncomeBill> getPageList() {
		return pageList;
	}
